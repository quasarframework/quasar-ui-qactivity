...
export default {
  ...
  setup () {
    ...
    const currentPackage = ref('QActivity')
    const versions = ref({})
    ...
    function queryReleases (page = 1) {
      ...
      xhrQuasar.addEventListener('load', function () {
        ...
        // sort by date
        packages.value.QActivity.sort((a, b) => {
      ...
      xhrQuasar.open('GET', `https://api.github.com/repos/quasarframework/quasar-ui-qactivity/releases?page=${ page }&per_page=100`)
      xhrQuasar.send()
    }
